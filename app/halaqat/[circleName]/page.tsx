  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const rawStudents = (studentsJson.students ?? []) as Student[]
        ...
        console.error("[halaqat/circle] Error fetching students:", error)
        setStudents([])
      } finally {
        setLoading(false)
      }
      ...
      <div className="min-h-screen flex flex-col bg-white" dir="rtl">
        ...
        <main className="flex flex-1 items-center justify-center">
          <div className="text-center">
            <h1 className="mb-4 text-4xl font-bold text-[#1a2332] md:text-5xl">{circleName}</h1>
            <p className="text-xl text-gray-600">لا يوجد طلاب في هذه الحلقة حالياً</p>
          </div>
        </main>
        <Footer />
      </div>
      ...
      <main className="flex-1 py-8 md:py-16">
        <div className="container mx-auto px-3 md:px-4">
          <div className="mb-8 text-center md:mb-16">
            <h1 className="text-3xl font-black text-[#173d3a] md:text-5xl">{circleName}</h1>
          </div>
