...
code = code.replace(/export default function TeacherManagement\(\)/g, 'export function GlobalTeachersDialog()');
code = code.replace(/const \[isLoading, setIsLoading\] = useState\(true\)/g, 'const [isLoading, setIsLoading] = useState(true)\n  const [isOpen, setIsOpen] = useState(true)\n  const handleClose = (open) => { if(!open) { setIsOpen(false); setTimeout(() => router.push("?"), 300) } }');

// We need to replace the entire return statement.
// We will just read the file and replace everything starting at return ( till the end.
const matchReturn = code.split('return (');
if (matchReturn.length > 1) {
  const newReturn = `
...
`;
  code = matchReturn[0] + 'return (' + newReturn;
}
